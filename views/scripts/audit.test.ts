...
# Containers
`;
		const result = loadView("docs/public/views/02-containers.md", content);
...
# missing sources
`;
		const result = loadView("docs/public/views/broken.md", content);
...
`;
	...
	it("surfaces an error when no frontmatter block is present", () => {
		const result = loadView("docs/public/views/broken.md", "# just a title\n");
		expect("error" in result).toBe(true);
		if ("error" in result) {
...
describe("auditViews", () => {
	const views: View[] = [
		{
			slug: "02-containers",
			path: "docs/public/views/02-containers.md",
			sources: ["shared/**", "service/src/**"],
		},
		{
			slug: "03-deployment",
			path: "docs/public/views/03-deployment.md",
			sources: ["extension/manifest.json"],
		},
		{
			slug: "06-threat-model",
			path: "docs/public/views/06-threat-model.md",
	...
	it("reports clean when no source globs match the diff", () => {
		...
		expect(report.clean.map((v) => v.slug)).toEqual([
			"02-containers",
			"03-deployment",
			"06-threat-model",
		]);

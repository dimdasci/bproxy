...
| Information disclosure | One agent sharing `BPROXY_HOME` enumerates another agent's live sessions or debug surfaces | Required `--nick`; daemon filters `session.list`, `debug.status`, `debug.last`, and `debug.log` by live session ownership | ADR-030; `sessions.ts`; `debug-actions.ts`; `routes/command.ts` |
| Information disclosure | Persisted logs reveal raw agent nicknames | Daemon never persists raw nick; logs use short per-instance `ownerHash = sha256(instanceSalt + nick).slice(0, 8)` instead | ADR-030; `owner-hash.ts`; daemon logger wiring |
...
| Denial of service | Runaway agent loops hammer its own session | Per-nick `minInterval` floor, sliding-window rate cap, metronome detection, and jittered error-path delay throttle request ingress and error loops | ADR-030; `safety.ts`; `routes/command.ts` |
...
| Elevation of privilege | Buggy agent commands another agent's live session after learning its id | Session ownership is stamped at creation and enforced on every session-bound command; mismatches fail with `SESSION_SCOPE_MISMATCH` | ADR-030; `sessions.ts`; `routes/session-actions.ts` |
...
| Trace / dedupe ring buffer | `debug.log` could expose stale or over-broad extension data | Trace is bounded in `chrome.storage.session`, queryable only through authenticated daemon forwarding, stamped with `session` + `extensionVersion`, and daemon-filtered to the requesting nick's live sessions only |

...
```bash
bproxy elements -s m4q7z2 --tab t1
```
...
```bash
bproxy click -s m4q7z2 --tab t1 --element ln1
```

Or target by CSS selector:

```bash
bproxy click -s m4q7z2 --tab t1 --selector 'a[href="/about"]'
```

## Fill a form

```bash
bproxy fill -s m4q7z2 --tab t1 --element el2 --value "hello@example.com" --method paste --world isolated
```
...
See [Fill Method Selection](../solution/shared.md) for the complete decision tree.

## Scroll

```bash
bproxy scroll -s m4q7z2 --tab t1 --direction down
```

Scroll a specific element:

```bash
bproxy scroll -s m4q7z2 --tab t1 --element el5 --direction down
```

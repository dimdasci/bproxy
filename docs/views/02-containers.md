...
flowchart LR
  ...
  BG <-- "persistent WS\nBproxyForwardedRequest { target.tabId }" --> Daemon
...
- **Three processes, four protocols.** The CLI talks HTTP to the Daemon. The Daemon talks WebSocket to the Extension's Background SW and forwards browser actions as `BproxyForwardedRequest` messages that already carry daemon-owned `target.tabId` metadata. The SW dispatches into Content Scripts via `chrome.scripting`. The Content Script reads and writes the page via the DOM. Two distinct authentication tokens secure the two daemon-facing routes ([ADR-010](../decisions.md#adr-010-websocket-auth-transport--two-token-model)).
...
- Inside the CLI: [auto/cli-components.svg](../auto/cli-components.svg) — generated by `pnpm views:regen`.
...
- What protects the wire and the token files: [06-threat-model](./06-threat-model.md) — DFD + STRIDE for the daemon and shipped extension surface.
- Inside the Extension: [auto/extension-components.svg](../auto/extension-components.svg) — generated by `pnpm views:regen`.
- Trust boundaries and where each process runs: [03-deployment](./03-deployment.md) — machine placement, local-only control plane, and browser/profile boundaries.

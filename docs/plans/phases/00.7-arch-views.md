...
**Tech Stack:** pnpm workspaces · Astro 5 · Astro Starlight 0.32+ · Mermaid 11 via `rehype-mermaid` · Zod (via `astro:content`) · Vitest · TypeScript 5 · `dependency-cruiser` (invoked by `views:regen`).
...
3. **Sync helpers ship complete, not partial.** Both `views:audit` and `views:regen` exist, have tests, and are wired as pnpm scripts. `views:regen` is a working no-op until Phase 1+ provides source code to scan.
...
- [ ] **Step 1: Add ADR-019 to `docs/decisions.md`**
...
- [ ] **Step 2: Add ADR-020 to `docs/decisions.md`**
...
- [ ] **Step 3: Verify both ADRs parse and link**
...
- [ ] **Step 1: Write `views/package.json`**
...
- [ ] **Step 2: Write `views/tsconfig.json`**
...
- [ ] **Step 3: Write `views/astro.config.mjs`**
...
- [ ] **Step 5: Create empty `views/src/styles/custom.css`**
...
**Purpose:** Wire `rehype-mermaid` so fenced ` ```mermaid ` blocks render to inline SVG at build time. Verify with the simplest possible diagram before authoring the real Container view.

`rehype-mermaid` uses Playwright in `img-svg` mode to render diagrams at build time. To avoid the Playwright dep, we use `pre-mermaid` mode, which preserves the source in the page and renders client-side via Mermaid's JS at runtime — matching what the brainstorm validated (CDN-only Mermaid rendering works).

- [ ] **Step 1: Update `views/astro.config.mjs` to register `rehype-mermaid`**
...
- [ ] **Step 2: Create a smoke-test page at `views/src/content/docs/index.md`**
...
- [ ] **Step 3: Run dev server and verify the diagram renders in the browser**
...
- [ ] **Step 4: Verify build succeeds with the Mermaid block**
...
git commit -m "build(views): wire rehype-mermaid + CDN runtime for fenced diagrams"
```
...
- [ ] **Step 1: Write `views/src/content.config.ts`**
...
- [ ] **Step 2: Write `views/src/lib/view-schema.ts`**
...
> **Note:** `astro:content` re-exports `zod` so views/scripts can import the same `viewSchema` without pulling Zod directly. The audit script uses this in Task 9.

- [ ] **Step 3: Delete the smoke-test landing page**
...
- [ ] **Step 4: Run `astro check` to validate schemas + verify collections load**
...
- [ ] **Step 5: Verify dev server picks up existing docs**
...
- [ ] **Step 1: Update sidebar config in `views/astro.config.mjs`**
...
- [ ] **Step 2: Run dev server and verify all docs are reachable**
...
> If a doc page errors with "missing title", that's an existing-doc gap — Starlight expects a `title:` frontmatter line in each MDX/MD. Existing bproxy docs lead with `# Title` instead. **Do not modify the existing prose docs in this phase.** Instead, in the next sub-step, configure Starlight to fall back to the first H1.

- [ ] **Step 3: Configure title fallback for prose docs without frontmatter**

Many existing prose docs (architecture.md, decisions.md, scenarios.md, solution/*.md) lead with `# Title` and have no frontmatter. Starlight's docs collection requires a `title:` frontmatter field by default. We adjust the schema, not the existing docs.
...
- [ ] **Step 4: Re-run dev server and verify**

```bash
pnpm docs:dev
```
Expected: all sidebar links resolve to rendered pages. No "missing title" errors.
...
- [ ] **Step 1: Write `docs/views/02-containers.md`**
...
- [ ] **Step 2: Verify the view renders with frontmatter validation passing**
...
- [ ] **Step 3: Negative test — break the frontmatter and confirm the build fails**
...
- [ ] **Step 1: Write `views/src/components/RelatedAdrs.astro`**
...
- [ ] **Step 2: Write `views/src/components/PageFrame.astro`**
...
- [ ] **Step 3: Register the component override in `views/astro.config.mjs`**
...
- [ ] **Step 4: Verify the footer renders on the Container view**
...
- [ ] **Step 5: Open the existing `docs/architecture.md` in the site**
...
- [ ] **Step 1: Write the failing test `views/scripts/audit.test.ts`**

```typescript
import { describe, expect, it } from 'vitest';
import { auditViews, type View } from './audit';
...
- [ ] **Step 3: Implement the pure function and CLI wrapper in `views/scripts/audit.ts`**
...
interface ViewFrontmatter {
  layer?: string;
  sources?: string[];
}

function parseFrontmatter(content: string): ViewFrontmatter {
  ...
  for (const raw of yaml.split('\n')) {
    ...
    if (line.startsWith('layer:')) layer = line.slice('layer:'.length).trim();
    if (line === 'sources:') { inSources = true; continue; }
    if (inSources) {
      if (line.startsWith('  - ')) sources.push(line.slice(4).trim());
      else if (line && !line.startsWith(' ')) inSources = false;
    }
  }
  return { layer, sources };
}

function discoverViews(viewsDir: string): View[] {
  const out: View[] = [];
  const walk = (dir: string): void => {
    for (const entry of readdirSync(dir)) {
      ...
      out.push({ slug, path: rel, sources: fm.sources ?? [] });
    }
  };
  walk(viewsDir);
  return out;
...
function formatReport(report: AuditReport): string {
  ...
  lines.push('');
  lines.push('Helpers exit 0 regardless of findings. Review each ⚠ and decide whether the view needs an update.');
  return lines.join('\n');
}
...
if (isMain) {
  ...
  const views = discoverViews(viewsDir);
...
> **Implementation note:** `parseFrontmatter` is intentionally small — it handles the two fields the audit script actually consumes (`layer`, `sources`). A full YAML parser is YAGNI here; if the schema grows, swap in `js-yaml` later. The Astro content collection (Task 5) handles the full Zod validation at build time, catching anything this parser would miss.
...
- [ ] **Step 5: Smoke-test the CLI against the current branch**
...
- [ ] **Step 1: Write the failing test `views/scripts/regen.test.ts`**
...
- [ ] **Step 3: Implement `views/scripts/regen.ts`**
...
- [ ] **Step 5: Smoke-test the CLI in current (sourceless) state**
...
## Task 11: CI workflow — fail PRs on broken Mermaid or schema violations

**Files:**
- Create: `.github/workflows/docs.yml`

**Purpose:** Run `pnpm install && pnpm docs:build && pnpm test` on every PR. The build step fails on Mermaid syntax errors and Zod schema violations (proven by Task 7 Step 3). No deploy.
...
- [ ] **Step 1: Local dev server renders the Container view as canonical diagram + prose docs sidebar-accessible**
...
- [ ] **Step 2: `views:audit` reports correctly on a synthetic diff**
...
- [ ] **Step 3: `views:regen` exits 0 with no source code**
...
- [ ] **Step 4: CI build passes locally**
...
- [ ] **Step 5: Confirm no stray TODO / FIXME / XXX in new files**
...
- [ ] **Step 6: Confirm spec + roadmap reflect what was built**
...
- [ ] **Step 7: Phase 0.7 done.** The architecture viewer ships. Phase 1 (Shared types and workspace scaffold) is unblocked; its plan can now be written knowing the monorepo skeleton already exists.

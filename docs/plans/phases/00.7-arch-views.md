...
**Tech Stack:** pnpm workspaces · Astro 5 · Astro Starlight 0.32+ · Mermaid 11 via `rehype-mermaid` · Zod (imported directly so the audit CLI can reuse the same schema as Astro) · `js-yaml` for frontmatter parsing · Vitest · TypeScript 5 · `dependency-cruiser` (invoked by `views:regen`).
...
- [ ] **Step 3: Implement `loadView`, `auditViews`, and the CLI wrapper in `views/scripts/audit.ts`**

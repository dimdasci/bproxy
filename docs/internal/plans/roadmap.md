...
| 7 | Distribution & installation | Package and document install/upgrade outside the monorepo | ✅ Done | [phases/07-distribution.md](./phases/07-distribution.md) |
...
**Done when:** a fresh paired setup can run `tab open -> navigate -> text -> links -> session close` without fake tab ids, raw Chrome ids, implicit shared `default` state, manual rebinding, or external HTML parsing; `session close` automatically closes session-owned Chrome tabs; Scenario 1 (Google research) runs autonomously to completion; Scenario 2 (authenticated feed snapshot) handles scroll/pause/`HUMAN_REQUIRED` correctly; Scenario 3 (form fill) fills a real application form to the user-review step; docs match the implemented command and response shapes; pre-commit hooks block commits that fail the selected fast gates.

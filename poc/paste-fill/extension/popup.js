...
function pageTaskFillAndRead(value) {
	...
	function findStartPostButton() {
		...
		for (let i = 0; i < list.length; i += 1) {
			...
			if (
				t.indexOf("start a post") >= 0 ||
				t.indexOf("create a post") >= 0 ||
				a.indexOf("start a post") >= 0 ||
				a.indexOf("create a post") >= 0
			)
				return el;
		}
		return null;
	...
	function findQuillInRoot(root) {
		...
		if (direct && direct.__quill)
			return { quill: direct.__quill, container: direct, route: "root.query.__quill" };
		...
		if (!composer) {
			return {
				...
				debug: {
					...
					shadowHosts: collectShadowHosts().length,
				},
			};
		}
		...
		if (!found) {
			...
			return {
				...
				debug: {
					...
					startPostText: startPostText,
					attempts: attempts,
					editorCount: root.querySelectorAll('.ql-editor, [contenteditable="true"][role="textbox"]')
						.length,
		...
		for (let k = 0; k < all.length; k += 1) {
			...
			if (el.__quill) {
				...
				return {
					ok: true,
					href: location.href,
					route: "root.star.__quill",
					editorClass: el.className,
					value: value,
				};
			}
		}
...
function pageTaskSnapshot() {
	...
	const selector =
		'input, textarea, select, button, a, [role="button"], [role="textbox"], [contenteditable="true"], .ql-editor, .ql-container';
	const elements = [];
